...
def find_event_to_order(ls_symbols, d_data, order_file='HW7_Order.csv'):
    df_close = d_data['close']
    df_mean = pd.rolling_mean(df_close, 20)   
    df_std = pd.rolling_std(df_close, 20)    
    df_boll_val = (df_close - df_mean) / df_std   
    ...
    for s_symbol in ls_symbols:
        for i in range(20, len(ldt_timestamps)):
            # Calc Event Logic
            f_equity_boll_today = df_boll_val[s_symbol].ix[ldt_timestamps[i]]
            f_equity_boll_yest = df_boll_val[s_symbol].ix[ldt_timestamps[i - 1]]
            f_market_bool_today = df_boll_val['SPY'].ix[ldt_timestamps[i]]
                
            
            if f_equity_boll_today < 1.5 and f_equity_boll_yest >= -1.0:
...
def sim_trade(df_stock_data, stock_orders, init_cash=100000):
...
if __name__ == '__main__':
    ...
    df_events = find_event_to_order(ls_symbols, d_data)
    ep.eventprofiler(df_events, d_data, i_lookback=20, i_lookforward=20,
                     s_filename='EventStudyHW07.pdf', b_market_neutral=True, b_errorbars=True,
                     s_market_sym='SPY')
    print '-------------------- Find Event Order End --------------------'
    
    
    print '-------------------- Simulate Trade Start --------------------'
    file_path = 'HW7_Order.csv'
